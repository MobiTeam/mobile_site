...
function createTimetableHTML(date, timetable, showEmptyFields){
	...
	if(currSubgr != "0" && !!timetable[dateNumbers]){
		...
		for(var key in timetable[dateNumbers]){
			if(timetable[dateNumbers][key].SUBGRUP == null || currSubgr == timetable[dateNumbers][key].SUBGRUP){
				clearArr.push(timetable[dateNumbers][key]);
			}
		}

		timetable[dateNumbers] = clearArr.length > 0 ? clearArr : undefined;
	...
	if(!!timetable[dateNumbers]){
		...
		for(var i = 1, numLession = 6; i <= numLession; i++) {
			...
			if (+item.PAIR == i) {
				...
				if (showEmptyFields) timetableHTML += '<tr class="timetable_tr">\
										<td class="date_td">' + numLessonsArr[i] + '</td>\
