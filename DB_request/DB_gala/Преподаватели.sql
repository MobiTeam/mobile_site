...
----------------------------------------------------------------------------------C“¿¬ » œ–≈œŒƒ¿¬¿“≈Àﬂ( ‡‰Ó‚‡ˇ ËÌÙÓÏ‡ˆËˇ Ó ÔÂÔÓ‰‡‚‡ÚÂÎÂ)
...
select RowNum rn, P.fStrTabn tab, P.fFio fio, V.path prof, A.fRate, A.fCategory, to_oradate(A.fAppointDate) datereg,P.fSex,to_oradate(P.fBornDate) fBornDate,
       D.fName, NVL(B.fName,'ÕÂ ÛÍ‡Á‡Ì') fBud, S.fCategory stavka,
       S.fcInf3, A.fcRef3, K.fNaiKat, 
       A.fLprizn, case when P.fcSovm = '8000000000000001'                     then '¬ÌÂ¯ÌÂÂ ÒÓ‚ÏÂÒÚËÚÂÎ¸ÒÚ‚Ó'
                       when A.fLprizn = 0                                     then 'ŒÒÌÓ‚ÌÓÂ ÏÂÒÚÓ ‡·ÓÚ˚'
                       when A.fLprizn = 3 and A.fKindApp = '80010000000286EC' then '—Ó‚ÏÂ˘ÂÌËÂ ‰ÓÎÊÌÓÒÚÂÈ'
                       when A.fLprizn = 3                                     then '¬ÌÛÚÂÌÌÂÂ ÒÓ‚ÏÂÒÚËÚÂÎ¸ÒÚ‚Ó' end prizn,
 ...
 where (A.fDisMissDate = 0 or trunc(to_oradate(A.fDisMissDate)) >= trunc(sysdate))
   ...
   --and  P.fFio$up like '¡”–À”÷ »… ¬À¿ƒ»Ã»– ¬À¿ƒ»Ã»–Œ¬»◊' 
order by flprizn
------------------------------------------------------------------------------------------“≈À≈‘ŒÕ€ » »’ Ã≈—“¿ Œ¡»“¿Õ»ﬂ(ÍÓÌÚ‡ÍÚ ËÌÙÓÏ‡ˆËˇ)
...
---------------------------------------------------------------------------—Œ“–”ƒÕ» » œŒ  ¿‘≈ƒ–≈
select rownum rn, tab, fio, prof, frate, fcategory, datereg, fname, fbud, stavka, fcinf3, fcref3, fnaikat, decode(flprizn,0,'ŒÒÌÓ‚ÌÓÂ','¬ÌÛÚÂÌÂÂ ÒÓ‚ÏÂÒÚËÚÂÎ¸ÒÚ‚Ó') prizn,t.ftarif 
       from tarstav t INNER JOIN
            (select 
                    p.ftabnmb tab, p.ffio$up fio, c.fname prof, a.frate, a.fcategory, to_oradate(a.fappointdate) datereg, c1.fname, decode(c2.fname,NULL,'ÕÂ ÔÓÒÚ‡‚ÎÂÌ',c2.fname) as fbud, s.fcategory stavka, s.fcinf3, a.fcref3, k.fnaikat, a.flprizn,a.ftariff 
                    from 
              ...
              where (A.fDisMissDate = 0 or trunc(to_oradate(A.fDisMissDate)) >= trunc(sysdate))
                ...
                WHERE flprizn=0 
                --  and prof=' ‡ÙÂ‰‡ ËÌÓÒÚ‡ÌÌ˚ı ˇÁ˚ÍÓ‚'
                   ORDER BY fio   
--------------------------------------------------------------- Œ““≈ƒ∆»--ÓÒÚ‡ÚÓÍ-‰‡Ú‡ Ó·ÌÓ‚ÎÂÌËˇ
select * from Dol_kott 
    where instr(
        upper(replace(replace(FIO,'',''),' ','')),
        upper(replace(replace('¡ÛÎÛˆÍËÈ ¬Î‡‰ËÏË','.',''),' ','')),1)>=1
