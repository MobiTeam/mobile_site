...
if(isset($_POST['login'])) {
	...
	$stmt=$conn->prepare("select 30 - (TO_CHAR(sysdate,'sssss') - TO_CHAR(DATE_INPUT,'sssss')) as LAST_TRYING 
		    			   ...
		    			   where login = :login
		    		         and flag = 0 
		    		         and DATE_INPUT > (sysdate - (30 / (24 * 60 * 60)))");
	...
	if (isset($_POST['password']) && (strlen($_POST['password'])>=1)) {
		...
		// попытка авторизации в качестве преподавателя				
		if ((auth($clearLogin."@".$ad_ugrasu['domain'], $_POST['password'], $ad_ugrasu) == 0)) {

		   // попытка автозироваться в качестве студента
		   if(auth($clearLogin."@".$ad_edu['domain'], $_POST['password'], $ad_edu) == 0) {
			...
			$query->execute(array('login' => $clearLogin,
				                   'pass' => $pass));
			...
			if(!$isset_in_base) {
			   ...
			   // если успешно авторизовался под студенческим логином
			   $data_user['is_student'] = '1';	
			}					   
		} else {}					
	...
	if(isset($serverRequest) && ($data_user['serverRequest'] == 'default')) {
		...
		// получение полной информации по авторизованному человеку
		...
		//СОТРУДНИК					   
		   case "0":
				...
				$query=$conn->prepare("select cast(FIO AS VARCHAR2(512)) as FIO
		...
		//СТУДЕНТ
		   ...
		   case "1":
		   	...
		   	$query=$conn->prepare("select cast(FFIO AS VARCHAR2(512)) as FFIO, cast(GRUP AS VARCHAR2(512)) as GRUP
			  						 from MV_STUD_APPOINT_ALL
									 where instr(upper(replace(replace(FFIO,'.',''),' ','')), upper(replace(replace(:FFIO,'.',''),' ','')), 1)=1");	
    ...
    $stmt->execute(array('login' => $clearLogin,'pass' => $pass,'num_group' => $num_group,'data_user' => $data_user['FIO']));					

	////////////////////////////////////////////////////
    $stmt=$conn->prepare("select ID from T_USERS where LOGIN = :login ");
	$stmt->execute(array('login' => $clearLogin));
